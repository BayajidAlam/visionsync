...
# Lighter check for app deployments — only needs SSH hop IPs, not full DB topology.
# Populate .env.infra by running: PULUMI_CONFIG_PASSPHRASE=<pass> make save-outputs
require-deploy:
> @test -n "$(BASTION_IP)" || (echo "$(RED)BASTION_IP empty — run: PULUMI_CONFIG_PASSPHRASE=<pass> make save-outputs$(NC)" && exit 1)
> @test -n "$(FRONTEND_EC2_IP)" || (echo "$(RED)FRONTEND_EC2_IP empty — run: PULUMI_CONFIG_PASSPHRASE=<pass> make save-outputs$(NC)" && exit 1)
> @test -n "$(BACKEND_EC2_IP)" || (echo "$(RED)BACKEND_EC2_IP empty — run: PULUMI_CONFIG_PASSPHRASE=<pass> make save-outputs$(NC)" && exit 1)

save-outputs: require-infra
> @echo "# Auto-generated by make save-outputs — do not commit" > .env.infra
> @printf 'BASTION_IP=%s\n' "$(BASTION_IP)" >> .env.infra
> @printf 'FRONTEND_EC2_IP=%s\n' "$(FRONTEND_EC2_IP)" >> .env.infra
> @printf 'BACKEND_EC2_IP=%s\n' "$(BACKEND_EC2_IP)" >> .env.infra
> @printf 'MONGO_PRIMARY_IP=%s\n' "$(MONGO_PRIMARY_IP)" >> .env.infra
> @printf 'MONGO_SECONDARY1_IP=%s\n' "$(MONGO_SECONDARY1_IP)" >> .env.infra
> @printf 'MONGO_SECONDARY2_IP=%s\n' "$(MONGO_SECONDARY2_IP)" >> .env.infra
> @printf 'REDIS_IP=%s\n' "$(REDIS_IP)" >> .env.infra
> @printf 'ALB_DNS=%s\n' "$(ALB_DNS)" >> .env.infra
> @printf 'CLOUDFRONT_DOMAIN=%s\n' "$(CLOUDFRONT_DOMAIN)" >> .env.infra
> @printf 'RAW_BUCKET=%s\n' "$(RAW_BUCKET)" >> .env.infra
> @printf 'PROCESSED_BUCKET=%s\n' "$(PROCESSED_BUCKET)" >> .env.infra
> @printf 'SQS_QUEUE_URL=%s\n' "$(SQS_QUEUE_URL)" >> .env.infra
> @printf 'MONGODB_URI=%s\n' "$(MONGODB_URI)" >> .env.infra
> @echo "$(GREEN)Saved infra outputs to .env.infra$(NC)"
...
deploy-server: preflight require-deploy prepare-key
...
deploy-client: preflight require-deploy prepare-key
...
logs-server-prod: prepare-key require-deploy
